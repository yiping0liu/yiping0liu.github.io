---
layout: page
title: 学术出版物
ref: publications
permalink: /publications_cn/
lang: 中文

jpubs:

    - title:   "Adapting reference vectors and scalarizing functions by growing neural gas to handle irregular Pareto fronts"
      authors:  
        - name: "Yiping Liu"
        - name: "Hisao Ishibuchi"
        - name: "Naoki Masuyama"
        - name: "Yusuke Nojima"     
      cauthor:
      journal: "IEEE Transactions on Evolutionary Computation"
      year:    "2019"
      impact:  "SCI, 影响因子: 8.124, 中科院JCR一区, 顶刊"
      pdf:     " "
      code:    "https://github.com/yiping0liu/DEA-GNG"

    - title:   "Topological clustering via adaptive resonance theory with information theoretic learning"
      authors:
        - name: "Naoki Masuyama"
        - name: "Chu Kiong Loo"
        - name: "Hisao Ishibuchi"
        - name: "Naoyuki Kubota"
        - name: "Yusuke Nojima"
        - name: "Yiping Liu"     
      cauthor:
      journal: "IEEE Access"
      year:    "2019"
      impact:  "SCI, 影响因子: 3.557, 中科院JCR二区"

    - title:   "A multi-modal multi-objective evolutionary algorithm using two-archive and recombination strategies"
      authors:  
        - name: "Yiping Liu"
        - name: "Gary G. Yen"
        - name: "Dunwei Gong"     
      cauthor:
      journal: "IEEE Transactions on Evolutionary Computation"
      year:    "2018"
      doi:     "10.1109/TEVC.2018.2879406"
      impact:  "SCI, 影响因子: 8.124, 中科院JCR一区, 顶刊"
      pdf:     "/pdf/TriMOEA-TA&R.pdf"
      code:    "https://github.com/yiping0liu/TriMOEA-TAnR"

    - title:   "A meta-objective approach for many-objective evolutionary optimization"
      authors:
        - name: "Dunwei Gong"
        - name: "Yiping Liu"
        - name: "Gary G. Yen"  
      cauthor: 1
      journal: "Evolutionary Computation"
      year:    "2018"
      doi:     "10.1162/evco_a_00243"
      impact:  "SCI, 影响因子: 2.388, 中科院JCR二区, 进化计算领域著名期刊"
      pdf:     "/pdf/MeO.pdf"
      code:    "https://github.com/yiping0liu/MeO"

    - title:   "A many-objective evolutionary algorithm using a one-by-one selection strategy"
      authors:  
        - name: "Yiping Liu"
        - name: "Dunwei Gong"  
        - name: "Jing Sun"
        - name: "Yaochu Jin"   
      cauthor:
      journal: "IEEE Transactions on Cybernetics"
      year:    "2017"
      volume:  "47"
      number:  "9"
      page:    "2689-2702"
      doi:     "10.1109/TCYB.2016.2638902"
      impact:  "SCI, 影响因子: 8.775, 中科院JCR一区, 顶刊"
      pdf:     "/pdf/1by1EA.pdf"
      code:    "https://github.com/yiping0liu/1by1EA"

    - title:   "Many-objective evolutionary optimization based on reference points"
      authors:  
        - name: "Yiping Liu"
        - name: "Dunwei Gong"  
        - name: "Xiaoyan Sun"
        - name: "Yong Zhang"   
      cauthor:
      journal: "Applied Soft Computing"
      year:    "2017"
      volume:  "50"
      number:  "1"
      page:    "344-355"
      doi:     "10.1016/j.asoc.2016.11.009"
      impact:  "SCI, 影响因子: 3.907, 中科院JCR二区"
      pdf:     "/pdf/RPEA.pdf"
      code:    "https://github.com/yiping0liu/RPEA"

    - title:   "Indicator-based set evolution particle swarm optimization for many-objective problems"
      authors:  
        - name: "Xiaoyan Sun"
        - name: "Yang Chen"
        - name: "Yiping Liu"
        - name: "Dunwei Gong"    
      cauthor:
      journal: "Soft Computing"
      year:    "2016"
      volume:  "20"
      number:  "6"
      page:    "2219-2232"
      doi:     "10.1007/s00500-015-1637-1"
      impact:  "SCI, 影响因子: 2.367, 中科院JCR三区"

    - title:   "Evolutionary algorithms with user’s preferences for solving hybrid interval multi-objective optimization problems"
      authors:
        - name: "Dunwei Gong"  
        - name: "Yiping Liu"  
        - name: "Xinfang Ji"
        - name: "Jing Sun"   
      cauthor: 1
      journal: "Applied Intelligence"
      year:    "2015"
      volume:  "43"
      number:  "3"
      page:    "676-694"
      doi:     "10.1007/s10489-015-0658-x"
      impact:  "SCI, 影响因子: 1.983, 中科院JCR三区"

    - title:   "基于目标分解的高维多目标并行进化优化方法"
      authors:
        - name: "巩敦卫"  
        - name: "刘益萍"  
        - name: "孙晓燕"
        - name: "韩玉艳"   
      cauthor: 1
      journal: "自动化学报"
      year:    "2015"
      volume:  "41"
      number:  "8"
      page:    "1438-1451"
      doi:     "10.16383/j.aas.2015.c140832"
      impact:  "EI, 中文顶级期刊"


cpubs:

    - title:   "Searching for local pareto optimal solutions: A case study on polygon-based problems"
      authors:  
        - name: "Yiping Liu"
        - name: "Hisao Ishibuchi"
        - name: "Yusuke Nojima"
        - name: "Naoki Masuyama"
        - name: "Yuyan Han"       
      cauthor:
      conference: "IEEE Congress on Evolutionary Computation"
      year:    "2019"
      organization: "IEEE"
      arate:   "录用率: 39% (口头报告)"
      abbr:    "CEC"
      pdf:     "/pdf/DNEAL.pdf"
      code:    "https://github.com/yiping0liu/DNEA"

    - title:   "Migrating birds optimization for lot-streaming flow shop scheduling problem"
      authors:
        - name: "Yuyan Han"
        - name: "Junqing Li"
        - name: "Yiping Liu"
        - name: "Lili Liu"
        - name: "Zhixin Zheng"      
      cauthor:
      conference: "IEEE Congress on Evolutionary Computation"
      year:    "2019"
      organization: "IEEE"
      arate:   "录用率: 39% (口头报告)"
      abbr:    "CEC"

    - title:   "Constrained multiobjective distance minimization problems"
      authors:
        - name: "Yusuke Nojima"
        - name: "Takafumi Fukase"
        - name: "Yiping Liu"
        - name: "Naoki Masuyama"
        - name: "Hisao Ishibuchi"      
      cauthor:
      conference: "The Genetic and Evolutionary Computation Conference"
      year:    "2019"
      organization: "ACM"
      arate:   "CCF C类, 录用率: 35%, 进化计算领域顶会"
      abbr:    "GECCO"

    - title:   "A double-niched evolutionary algorithm and its behavior on polygon-based problems"
      authors:  
        - name: "Yiping Liu"
        - name: "Hisao Ishibuchi"
        - name: "Yusuke Nojima"
        - name: "Naoki Masuyama"
        - name: "Ke Shang"       
      cauthor:
      conference: "International Conference on Parallel Problem Solving from Nature"
      year:    "2018"
      page:    "262-273"
      organization: "Springer"
      doi:     "10.1007/978-3-319-99253-2_21"
      arate:   "CCF B类, 录用率: 38.6%, 进化计算领域顶会"
      abbr:    "PPSN"
      pdf:     "/pdf/DNEA.pdf"
      code:    "https://github.com/yiping0liu/DNEA"

    - title:   "Improving 1by1EA to handle various shapes of Pareto fronts"
      authors:  
        - name: "Yiping Liu"
        - name: "Hisao Ishibuchi"
        - name: "Yusuke Nojima"
        - name: "Naoki Masuyama"
        - name: "Ke Shang"       
      cauthor:
      conference: "International Conference on Parallel Problem Solving from Nature"
      year:    "2018"
      page:    "311-322"
      organization: "Springer"
      doi:     "10.1007/978-3-319-99253-2_25"
      arate:   "CCF B类, 录用率: 38.6%, 进化计算领域顶会"
      abbr:    "PPSN"
      pdf:     "/pdf/1by1EA2.pdf"
      code:    "https://github.com/yiping0liu/1by1EA"

    - title:   "A new R2 indicator for better hypervolume approximation"
      authors:
        - name: "Ke Shang"
        - name: "Hisao Ishibuchi"
        - name: "Minling Zhang"
        - name: "Yiping Liu"     
      cauthor:
      conference: "The Genetic and Evolutionary Computation Conference"
      year:    "2018"
      page:    "745-752"
      organization: "ACM"
      arate:   "CCF C类, 录用率: 38%, 进化计算领域顶会"
      abbr:    "GECCO"
      doi:     "10.1145/3205455.3205543"

    - title:   "Topological Kernel Bayesian ARTMAP"
      authors:
        - name: "Naoki Masuyama"
        - name: "Chu Kiong Loo"
        - name: "Hisao Ishibuchi"
        - name: "Yusuke Nojima"
        - name: "Yiping Liu"     
      cauthor:
      conference: "World Automation Congress"
      year:    "2018"
      page:    "316-321"
      organization: "IEEE"
      abbr:    "WAC"
      doi:     "10.23919/WAC.2018.8430302"

    - title:   "A reference points-based evolutionary algorithm for many-objective optimization"
      authors:  
        - name: "Yiping Liu"
        - name: "Dunwei Gong"  
        - name: "Xiaoyan Sun"
        - name: "Yong Zhang"        
      cauthor:
      conference: "The Genetic and Evolutionary Computation Conference"
      year:    "2014"
      page:    "1053-1056"
      organization: "ACM"
      doi:     "10.1145/2598394.2605674"
      abbr:    "GECCO Student Workshop"



---

在<a href="https://scholar.google.com/citations?user=2eX_3XEAAAAJ&hl"><U>Google Scholar</U></a>浏览.<br />
从<a href="https://github.com/yiping0liu"><U>GitHub</U></a>下载代码.<br />
下载<a href="/pdf/YLiu.bib"><U>BibTex</U></a>引用格式.



### 期刊论文

(<img src="/images/mail.png" align="bottom" height="10pxm">表示通信作者)

{% for pub in page.jpubs %}

<table cellpadding="6px">
  <tr valign="top">
    <td>[J{{forloop.index}}]</td>

    <td>
    <font color="#922B21"><b>{{pub.title}}</b></font><br />

    {% for author in pub.authors  %}
    {% if author.name == "Yiping Liu"%}
    <b>{{author.name}}</b>{% if pub.cauthor %} <img src="/images/mail.png" align="middle" height="10pxm">{% endif %}{% else %}{%if author.name == "刘益萍"%}<b>{{author.name}}</b>{% if pub.cauthor %} <img src="/images/mail.png" align="middle" height="10pxm">{% endif %}{% else %}{{author.name}}{% endif %}{% endif %}{% if forloop.last %}{% else %},{% endif %}{% endfor %}<br />

    <i>{{pub.journal}}</i>,
    {% if pub.volume %}
    vol.{{pub.volume}}, no. {{pub.number}}, pp.{{pub.page}}, {{pub.year}}.
    {% else %}
    {% if pub.doi %}
    {{pub.year}} [Online].
    {% else %}
    {{pub.year}} [Accepted].
    {% endif %}
    {% endif %}
    <br />
    {% if pub.doi %}
    <a href="https://doi.org/{{pub.doi}}">[DOI: {{pub.doi}}]</a>
    {% endif %}
    {% if pub.pdf %}
    <a href="{{pub.pdf}}">[PDF]</a>
    {% endif %}
    {% if pub.code %}
    <a href="{{pub.code}}">[Code]</a>
    {% endif %}
    {% if pub.code%}
    <br />
    {% else %}
    {% if pub.doi%}
    <br />
    {% else %}
    {% if pub.pdf%}
    <br />
    {% endif %}
    {% endif %}
    {% endif %}
    {% if pub.impact %}
    <font color="#1F618D">{{pub.impact}}</font>
    {% endif %}
    </td>
  </tr>
</table>
{% endfor %}




### 会议论文

{% for pub in page.cpubs %}

<table cellpadding="6px">
  <tr valign="top">
    <td>[C{{forloop.index}}]</td>

    <td>
    <font color="#922B21"><b>{{pub.title}}</b></font><br />

    {% for author in pub.authors  %}
    {% if author.name == "Yiping Liu" %}
    <b>{{author.name}}</b>{% if pub.cauthor %}<b>@</b>{% endif %}{% else %}{{author.name}}{% endif %}{% if forloop.last %}{% else %},{% endif %}{% endfor %}<br />
    in <i>{{pub.conference}}</i> ({{pub.abbr}}). {{pub.organization}}, {{pub.year}}
    {% if pub.page %}
    , pp. {{pub.page}}.
    {% else %}
    [in press].
    {% endif %}
    <br />
    {% if pub.doi %}
    <a href="https://doi.org/{{pub.doi}}">[DOI: {{pub.doi}}]</a>
    {% endif %}
    {% if pub.pdf %}
    <a href="{{pub.pdf}}">[PDF]</a>
    {% endif %}
    {% if pub.code %}
    <a href="{{pub.code}}">[Code]</a>
    {% endif %}
    {% if pub.code%}
    <br />
    {% else %}
    {% if pub.doi%}
    <br />
    {% else %}
    {% if pub.pdf%}
    <br />
    {% endif %}
    {% endif %}
    {% endif %}
    {% if pub.arate %}
    <font color="#1F618D">{{pub.arate}}</font>
    {% endif %}

    </td>
  </tr>
</table>
{% endfor %}
